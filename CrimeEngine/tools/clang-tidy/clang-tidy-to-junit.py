#https://github.com/PSPDFKit-labs/clang-tidy-to-junit/blob/master/clang-tidy-to-junit.py
#!/usr/bin/env python3
...
class ClangTidyConverter:
    ...
    #Check if the line has an source file in it.
    ...
    def get_junit_data(self):

        #Escape any optional illigal characters for XML.
        escape_illigal_xml_chars = {
            '"' : "&apos;"
        }
...
<testsuites id="1" name="Clang-Tidy" tests="{error_count}" errors="{error_count}" failures="0" time="0">""".format(error_count=len(self.errors)))
        ...
        for file, errorIterator in itertools.groupby(sorted_errors, key=lambda x: x.file):
            ...
            data += ("""\n    <testsuite errors="{error_count}" name="{file}" tests="{error_count}" failures="0" time="0">"""
                              .format(error_count=error_count, file=escape(file, escape_illigal_xml_chars)))
            for error in errors:
        ...
        </testcase>""".format(id="[{}/{}] {}".format(error.line, error.column, error.error_identifier), message=escape(error.error, escape_illigal_xml_chars),
                              htmldata=escape(error.description, escape_illigal_xml_chars)))
            data += ("\n    </testsuite>\n")
        data += ("</testsuites>\n")
        return data
    ...
    def process_error(self, error_array):
        ...
        file_path = result.group(0).replace(self.basename, "")
        error = ErrorDescription(file_path, int(result.group(1)), int(
            result.group(2)), result.group(3), result.group(4), "\n".join(error_array[1:]))

        #Check if this error already exists!
        #Exlucluding the description since that could contain multiple lines.
        for e in self.errors:
            if (e.line == error.line and e.column == error.column and 
                e.error_identifier == error.error_identifier and e.error == error.error):
                return
    ...
    def convert(self, input_file, output_file):
        ...
        for line in input_file:
            # If the line starts with a path and ends with .cpp it is about a file. 
            if self.file_regex.search(line[0:256]):
                ...
                # If the line didn't start with a file cpp file and we have a `current_error`, we simply append
        ...
        data = self.get_junit_data()

        if self.validate(data) is False:
            logging.error("XML data wasn't correctly parsed something went wrong!")
