...
cr::DX12_DepthStencilBuffer::DX12_DepthStencilBuffer(DX12_Device& a_device, DX12_CommandList& a_cmd, int a_w, int a_h)
{
	...
	m_dsv = new DX12_Descriptor(D3D12_DESCRIPTOR_HEAP_TYPE_DSV, dvsDesc, a_device);
	SetBuffer(a_device, a_cmd, a_w, a_h);
}

cr::DX12_DepthStencilBuffer::~DX12_DepthStencilBuffer()
{
	delete m_dsv;
	m_dsBuffer.ReleaseAndGetAddressOf();
}

D3D12_CPU_DESCRIPTOR_HANDLE cr::DX12_DepthStencilBuffer::DepthStencilView() const
{
	return m_dsv->GetHeap()->GetCPUDescriptorHandleForHeapStart();
}

void cr::DX12_DepthStencilBuffer::SetBuffer(DX12_Device& a_device, DX12_CommandList& a_cmd, int a_w, int a_h)
{
	m_dsBuffer.Reset();
